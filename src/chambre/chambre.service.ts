...
export class ChambreService {
  ...
  async update(id: number, updateChambreDto: UpdateChambreDto): Promise<Chambre> {
    const chambre = await this.chambreRepository.preload({id, ...updateChambreDto});
    if(!chambre){
      throw new NotFoundException(`la chambre d'id ${id} est introuvable !`);
    }
  ...
  async restore(id: number) {
    ...
    if(response.affected === 0){
      throw new NotFoundException(`la chambre d'id ${id} est introuvable ou n'a pas été désactivé !`); 
    }
